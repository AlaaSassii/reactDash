  ...
  <div class="q-pa-md">
    ...
    <q-dialog
      ...
      <q-card>
        <q-toolbar class="bg-primary text-white text-bold q-pa-md">
          <q-toolbar-title>Add New Category</q-toolbar-title>
    ...
    <div style="min-height: 100%;max-height: 100%" class="row">
      <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
        <q-input :style="$props.isMobileView?'':'min-width: 50%;max-width: 50%'" v-model="search" label="Search For Category" outlined class="shadow-1"></q-input>
      </div>
      <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
...
export default {
  ...
  data(){
    return{
      ...
      warehouseColumn:[
        ...
        { name: 'Type', align: 'center', label: 'Type', field: row => row.type, sortable: true },
      ],
      dataMode:'add',
      selectedId:'',
      api: client,
  ...
  methods:{
    deleteItem(){
      this.api.putData('category/delete/'+this.selectedId,{},"Category Deleted").then((resp)=>{
        this.getItem()
      })
    },
    ...
    getItem(){
      this.api.getData('category/get').then((response)=>{
        this.itemData = response
      })
    },
    OpenAddDialog(){
      this.dataMode = 'add'
      this.formData = this.form.category_form
      this.dialogOpened = true
    },
    saveData(data){
      if(this.dataMode === 'add'){
        let saveData = this.form.createPostObject(data)
        this.api.postData('category',saveData,'Category Added').then(()=>{
          this.getItem()
