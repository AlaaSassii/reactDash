  ...
  <div class="q-pa-md">
    ...
    <q-dialog
      ...
      <q-card>
        <q-toolbar class="bg-primary text-white text-bold q-pa-md">
          <q-toolbar-title>Add New Site</q-toolbar-title>
    ...
    <div style="min-height: 100%;max-height: 100%" class="row">
      <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
        <q-input :style="$props.mobileView?'':'min-width: 50%;max-width: 50%'" v-model="search" label="Search For Sites" outlined class="shadow-1"></q-input>
      </div>
      <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
      ...
      <div class="col-12 q-pt-lg">
        <div class="row">
          <div :key="ind" class="col-lg-4 col-md-4 col-sm-6 col-xs-12 q-pa-sm" v-for="(item,ind) in itemData">
            <q-card @click="rowClicked(item)" style="min-height: 100%" class="my-card cursor-pointer">
              <q-img
                ...
                basic
              >
                <div class="absolute-bottom text-h6 text-bold text-center">
                  {{item.name}}
                </div>
              </q-img>
              <q-img
              ...
              <q-card-section class="q-pb-none">
                <div class="text-h6 text-bold font-bold text-secondary">{{item.name}}</div>
                <div class="text-caption">{{item.city}}</div>
              </q-card-section>
              <q-card-section class="q-pt-sm">
                <div class="q-pt-sm" v-if="item.state"><b>State: </b> {{item.state}}</div>
                <div class="q-pt-sm" v-if="item.street"><b>Street: </b>{{item.street}}</div>
                <div class="q-pt-sm" v-if="item.total_amount"><b>Total Amount In Site: </b>{{item.total_amount}}</div>
                <div class="q-pt-sm" v-if="item.total_stock"><b>Total Stock In Site: </b>{{item.total_stock}}</div>
              </q-card-section>
            </q-card>
          </div>
        </div>
...
export default {
  ...
  data(){
    return{
      ...
      warehouseColumn:[
        ...
        { name: 'Starting Date', align: 'center', label: 'Starting Date', field: row=>row.starting_date?row.starting_date.substring(0,10):'', sortable: true },
        { name: 'Total Amount', align: 'center', label: 'Total Amount', field: row=>row.total_amount?'Rs. '+row.total_amount:'Rs. 0', sortable: true },
      ],
      dataMode:'add',
      selectedId:'',
      currentPage:1,
  ...
  methods:{
    ...
    getInventoryHistory(){
      this.clientState.getData('site/history/'+this.selectedId+'/page/'+(this.inventoryCurrentPage-1)).then((resp)=>{
    ...
    editSite(){
      this.clientState.getData('site/get/id/'+this.selectedId).then((resp)=>{
        this.dataMode = 'edit'
        this.formData = this.form.createEditObject(this.form.site_form,resp)
        this.dialogOpened = true
      })
    },
    deleteWarehouse(){
      this.clientState.putData('site/delete/'+this.selectedId,{},"Site Deleted").then(()=>{
        this.getWarehouse()
      })
    },
    searchWarehouse(){
      this.clientState.getData('site/search/'+this.search).then((response)=>{
    ...
    getWarehouse(){
      this.clientState.getData('site/get/'+(this.currentPage-1)).then((response)=>{
        this.itemData = response.data
        this.currentPage = response.currentPage
        this.totalPage = response.totalPage
      })
    ...
    OpenAddDialog(){
      this.dataMode = 'add'
      this.formData = this.form.site_form
      this.dialogOpened = true
    },
    saveData(data){
      if(this.dataMode === 'add'){
        let saveData = this.form.createPostObject(data)
        this.clientState.postData('site',saveData,'Site Added').then(()=>{
          this.getWarehouse()
        ...
        let saveData = this.form.createPostObject(data)
        this.clientState.putData('site/edit/'+this.selectedId,saveData,'Site Updated').then(()=>{
          this.getWarehouse()
          this.dialogOpened = false
        })
      }
